...
import in.ults.ipms.databinding.FragmentBridgeDetailsBinding;
...
public class BridgeDetailsFragment extends BaseFragment<FragmentBridgeDetailsBinding> implements IBridgeDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = BridgeDetailsFragment.class.getSimpleName();

    public static final int ERROR_TYPE_BRIDGE_NAME = 1;
    public static final int ERROR_TYPE_PLACE = 2;
    public static final int ERROR_TYPE_BRIDGE_MATERIAL = 3;
    ...
    @Inject
    IBridgeDetailsPresenter<IBridgeDetailsView, IBridgeDetailsInteractor> presenter;


    CommonSpinnerAdapter<UtilitySpinnerResponse.MaintainedBy> maintainedByAdapter;
    CommonSpinnerAdapter<UtilitySpinnerResponse.BridgeSubType> bridgeSubTypeAdapter;
    CommonSpinnerAdapter<UtilitySpinnerResponse.BridgeMaterial> bridgeMaterialAdapter;
    ...
    @Override
    protected FragmentBridgeDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentBridgeDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.bridge_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgBridge.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null && AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues() != null) {
            ...
            maintainedByAdapter = CommonSpinnerAdapter.setAdapter(getBaseActivity(), getViewBinding().srBridgeMaintainedBy, AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues().getMaintainedBy());
            bridgeMaterialAdapter = CommonSpinnerAdapter.setAdapter(getBaseActivity(), getViewBinding().srBridgeMaterial, AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues().getBridgeMaterial());
            bridgeSubTypeAdapter = CommonSpinnerAdapter.setAdapter(getBaseActivity(), getViewBinding().srBridgeSubType, AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues().getBridgeSubType());
        }
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getBridge());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvBridgeDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvBridgeDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        String bridgeName = Objects.requireNonNull(getViewBinding().etBridgeName.getText()).toString().trim();
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_BRIDGE_NAME:
                getViewBinding().layoutBridgeName.setError(error);
                getViewBinding().layoutBridgeName.requestFocus();
                break;
            case ERROR_TYPE_PLACE:
                getViewBinding().layoutBridgePlace.setError(error);
                getViewBinding().layoutBridgePlace.requestFocus();
                break;
            case ERROR_TYPE_BRIDGE_MATERIAL:
                getViewBinding().layoutBridgeMaterial.setError(error);
                getViewBinding().layoutBridgeMaterial.requestFocus();
                break;
            case ERROR_TYPE_SUB_TYPE:
                getViewBinding().layoutBridgeSubType.setError(error);
                getViewBinding().layoutBridgeSubType.requestFocus();
                break;
            case ERROR_TYPE_CARRIAGE_WIDTH:
                getViewBinding().layoutBridgeCarriageWidth.setError(error);
                getViewBinding().layoutBridgeCarriageWidth.requestFocus();
                break;
            ...
            case ERROR_TYPE_MAINTAINED_BY:
                getViewBinding().layoutBridgeMaintainedBy.setError(error);
                getViewBinding().layoutBridgeMaintainedBy.requestFocus();
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutBridgeName.setErrorEnabled(false);
        getViewBinding().layoutBridgePlace.setErrorEnabled(false);
        getViewBinding().layoutBridgeMaterial.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null) {
            UtilityAssets.Bridge bridgeDetails = AppCacheData.getOurInstance().getBuildingAssetData().getBridgeDetails();
            if (bridgeDetails != null) {
                getViewBinding().etBridgeName.setText(bridgeDetails.getBridgeName());
                ...
                maintainedByAdapter.setContent(String.valueOf(bridgeDetails.getMaintainedBy()));
                wardAdapter.setContent(String.valueOf(bridgeDetails.getWard()));
                GeomPoint geom = bridgeDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+bridgeDetails.getPhoto1(), null, bridgeDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_BRIDGE, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgBridge.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
