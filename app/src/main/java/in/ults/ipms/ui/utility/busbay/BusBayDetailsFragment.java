...
import in.ults.ipms.databinding.FragmentBusBayDetailsBinding;
...
public class BusBayDetailsFragment extends BaseFragment<FragmentBusBayDetailsBinding> implements IBusBayDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = BusBayDetailsFragment.class.getSimpleName();
    ...
    public static BusBayDetailsFragment newInstance() {
        return new BusBayDetailsFragment();
    }

    @Inject
    IBusBayDetailsPresenter<IBusBayDetailsView, IBusBayDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentBusBayDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentBusBayDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.bus_bay_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgBusBay.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                for (DashboardResponse.BaseLayers layer : baseLayerData) {
                    ...
                    String[] place = layer.getLayerName().split(":");
                    String layerPlace = place.length > 0 ? place[place.length - 1] : "";
                    childLayers.setLabel(layer.getLabel());
                    childLayers.setLayerType(layer.getLayerType());
                    childLayers.setLayerName(layerPlace);
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getBusBay());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvBusBayDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvBusBayDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    public void plotPoint(double latitude, double longitude) {
        try {
            ...
            BitmapDrawable pinStarBlueDrawable = (BitmapDrawable) ContextCompat.getDrawable(getBaseActivity(), R.mipmap.marker_road_junction);
    ...
    void submitOnClick() {
        String place = Objects.requireNonNull(getViewBinding().etBusBayPlace.getText()).toString().trim();
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_PLACE:
                getViewBinding().layoutBusBayPlace.setError(error);
                getViewBinding().layoutBusBayPlace.requestFocus();
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutBusBayPlace.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null) {
            UtilityAssets.BusBay busBayDetails = AppCacheData.getOurInstance().getBuildingAssetData().getBusBayDetails();
            if (busBayDetails != null) {
                getViewBinding().etBusBayPlace.setText(busBayDetails.getBusBayPlace());
                getViewBinding().etRemarks.setText(busBayDetails.getRemarks());
                wardAdapter.setContent(String.valueOf(busBayDetails.getWard()));
                GeomPoint geom = busBayDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() + "" + busBayDetails.getPhoto1(), null, busBayDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_BUS_BAY, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgBusBay.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
