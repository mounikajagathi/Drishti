...
public class BusStopDetailsPresenter<V extends IBusStopDetailsView, I extends IBusStopDetailsInteractor>
        extends BasePresenter<V, I> implements IBusStopDetailsPresenter<V, I> {

    private final BaseActivity baseActivity;

    @Inject
    public BusStopDetailsPresenter(I mMvpInteractor, SchedulerProvider schedulerProvider, CompositeDisposable compositeDisposable, BaseActivity baseActivity) {
    ...
    public void validateData(String place, String wardNo, String remarks, String photo, double locationLatitude, double locationLongitude) {
        ...
        if (CommonUtils.isNullString(place)) {
            getMvpView().showErrors(BusStopDetailsFragment.ERROR_TYPE_PLACE, baseActivity.getResources().getString(R.string.err_bus_stop_details_place));
            return;
        }
        if (CommonUtils.isNullString(wardNo)) {
            getMvpView().showErrors(BusStopDetailsFragment.ERROR_TYPE_WARD_NO, baseActivity.getResources().getString(R.string.err_bus_stop_details_ward_number));
            return;
        }
        if (CommonUtils.isNullString(remarks)) {
            getMvpView().showErrors(BusStopDetailsFragment.ERROR_TYPE_REMARKS, baseActivity.getResources().getString(R.string.err_bus_stop_details_remarks));
            return;
        }
        if (CommonUtils.isNullString(photo)) {
            getMvpView().showErrors(BusStopDetailsFragment.ERROR_TYPE_PHOTO, baseActivity.getResources().getString(R.string.err_bus_stop_details_photo_1));
            return;
        }

        if (locationLatitude == 0.0 || locationLongitude == 0.0) {
            getMvpView().showErrors(BusStopDetailsFragment.ERROR_TYPE_LOCATION, baseActivity.getResources().getString(R.string.err_bus_stop_details_location));
            return;
        }
    ...
    void addUpdateContent(String place, String wardNo, String remarks, String photo, double locationLatitude, double locationLongitude) {
        ...
        if (AppCacheData.getOurInstance().isAssetUpdate()) {
            if (AppCacheData.getOurInstance().getBuildingAssetData() != null &&
                    AppCacheData.getOurInstance().getBuildingAssetData().getBusStopDetails() != null) {
                FetchAssetDataResponse.Data data = AppCacheData.getOurInstance().getBuildingAssetData();
                data.getBusStopDetails().setPlace(place);
                data.getBusStopDetails().setWard(Long.parseLong(wardNo));
                data.getBusStopDetails().setRemarks(remarks);
                data.getBusStopDetails().setPhoto1(photo);
                data.getBusStopDetails().setGeom(geom);
                data.getBusStopDetails().setUpdationStatus(AppConstants.UPDATION_STATUS_UPDATE);
                saveAssetDetails(data);
            }
        } else {
            FetchAssetDataResponse.Data data = new FetchAssetDataResponse.Data();
            UtilityAssets.BusStop details = new UtilityAssets.BusStop();
            ...
            data.setBusStopDetails(details);
            saveAssetDetails(data);
        }
    }
