...
import in.ults.ipms.databinding.FragmentCulvertDetailsBinding;
...
public class CulvertDetailsFragment extends BaseFragment<FragmentCulvertDetailsBinding> implements ICulvertDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = CulvertDetailsFragment.class.getSimpleName();
    ...
    @Inject
    ICulvertDetailsPresenter<ICulvertDetailsView, ICulvertDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentCulvertDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentCulvertDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.culvert_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgCulvert.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getCulvert());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvCulvertDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvCulvertDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    public void plotPoint(double latitude, double longitude) {
        try {
            ...
            BitmapDrawable pinStarBlueDrawable = (BitmapDrawable) ContextCompat.getDrawable(getBaseActivity(), R.mipmap.marker_culvert);
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_CULVERT_NAME:
                getViewBinding().layoutCulvertName.setError(error);
                getViewBinding().layoutCulvertName.requestFocus();
                break;
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null) {
            UtilityAssets.Culvert culvertDetails = AppCacheData.getOurInstance().getBuildingAssetData().getCulvertDetails();
            if (culvertDetails != null) {
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+culvertDetails.getPhoto1(), null, culvertDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_CULVERT, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageUploadSuccess(String imagePath, String imageType, String response) {
        ...
        Glide.with(this)
                ...
                .into(getViewBinding().imgCulvert);
    }
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgCulvert.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
