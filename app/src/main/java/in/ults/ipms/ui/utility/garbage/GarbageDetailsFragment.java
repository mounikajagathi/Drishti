...
import in.ults.ipms.databinding.FragmentGarbageDetailsBinding;
...
public class GarbageDetailsFragment extends BaseFragment<FragmentGarbageDetailsBinding> implements IGarbageDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = GarbageDetailsFragment.class.getSimpleName();
    ...
    public static GarbageDetailsFragment newInstance() {
        return new GarbageDetailsFragment();
    }

    @Inject
    IGarbageDetailsPresenter<IGarbageDetailsView, IGarbageDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentGarbageDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentGarbageDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.garbage_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgGarbage.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getGarbage());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvGarbageDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvGarbageDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        String place = Objects.requireNonNull(getViewBinding().etGarbagePlace.getText()).toString().trim();
        String capacity = Objects.requireNonNull(getViewBinding().etGarbageCategory.getText()).toString().trim();
        ...
        presenter.validateData(place, capacity, wardNo, remarks, photo, latitude, longitude);
    }
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_PLACE:
                getViewBinding().layoutGarbagePlace.setError(error);
                getViewBinding().layoutGarbagePlace.requestFocus();
                break;
            case ERROR_TYPE_CATEGORY:
                getViewBinding().layoutGarbageCategory.setError(error);
                getViewBinding().layoutGarbageCategory.requestFocus();
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutGarbagePlace.setErrorEnabled(false);
        getViewBinding().layoutGarbageCategory.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null ) {
            UtilityAssets.Garbage garbageDetails = AppCacheData.getOurInstance().getBuildingAssetData().getGarbageDetails();
            if (garbageDetails != null) {
                getViewBinding().etGarbagePlace.setText(garbageDetails.getPlace());
                getViewBinding().etGarbageCategory.setText(String.valueOf(garbageDetails.getCategory()));
                getViewBinding().etRemarks.setText(garbageDetails.getRemarks());
                wardAdapter.setContent(String.valueOf(garbageDetails.getWard()));
                GeomPoint geom = garbageDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+garbageDetails.getPhoto1(), null, garbageDetails.getPhoto1());
//                Log.d("base", new Gson().toJson(AppCacheData.getOurInstance().getImageBaseURL() ));
//                Log.d("base1....", new Gson().toJson(garbageDetails.getPhoto1()));
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_GARBAGE, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageUploadSuccess(String imagePath, String imageType, String response) {
        ...
        Glide.with(this)
                ...
                .into(getViewBinding().imgGarbage);
    }
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgGarbage.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
