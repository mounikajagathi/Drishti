...
import in.ults.ipms.databinding.FragmentRoadHumpDetailsBinding;
...
public class RoadHumpDetailsFragment extends BaseFragment<FragmentRoadHumpDetailsBinding> implements IRoadHumpDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = RoadHumpDetailsFragment.class.getSimpleName();
    ...
    @Inject
    IRoadHumpDetailsPresenter<IRoadHumpDetailsView, IRoadHumpDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentRoadHumpDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentRoadHumpDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.road_hump_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgRoadHump.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getRoadHump());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvRoadHumpDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvRoadHumpDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        String place = Objects.requireNonNull(getViewBinding().etRoadHumpPlace.getText()).toString().trim();
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_PLACE:
                getViewBinding().layoutRoadHumpPlace.setError(error);
                getViewBinding().layoutRoadHumpPlace.requestFocus();
                break;
            case ERROR_TYPE_NAME:
                getViewBinding().layoutRoadHumpName.setError(error);
                getViewBinding().layoutRoadHumpName.requestFocus();
                break;
            case ERROR_TYPE_LENGTH:
                getViewBinding().layoutRoadHumpLength.setError(error);
                getViewBinding().layoutRoadHumpLength.requestFocus();
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutRoadHumpPlace.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null) {
            UtilityAssets.RoadHump humpDetails = AppCacheData.getOurInstance().getBuildingAssetData().getRoadHumpDetails();
            if (humpDetails != null) {
                getViewBinding().etRoadHumpPlace.setText(humpDetails.getPlace());
                getViewBinding().etRoadHumpName.setText(humpDetails.getRoadName());
                getViewBinding().etRoadHumpLength.setText(String.valueOf(humpDetails.getLength()));
                getViewBinding().etRoadHumpWidth.setText(String.valueOf(humpDetails.getWidth()));
                getViewBinding().etRemarks.setText(humpDetails.getRemarks());
                wardAdapter.setContent(String.valueOf(humpDetails.getWard()));
                GeomPoint geom = humpDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+humpDetails.getPhoto1(), null, humpDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_ROAD_HUMP, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageUploadSuccess(String imagePath, String imageType, String response) {
        ...
        Glide.with(this)
                ...
                .into(getViewBinding().imgRoadHump);
    }
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgRoadHump.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
