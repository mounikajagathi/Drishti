...
import in.ults.ipms.databinding.FragmentRoadMapBinding;
import in.ults.ipms.singletons.AppCacheData;
import in.ults.ipms.ui.base.BaseFragment;
import in.ults.ipms.ui.mapoverlays.MapOverlayBottomSheet;
import in.ults.ipms.utils.AppConstants;

public class RoadMapFragment extends BaseFragment<FragmentRoadMapBinding> implements IRoadMapView {

    public static final String TAG = RoadMapFragment.class.getSimpleName();

    @Inject
    IRoadMapPresenter<IRoadMapView, IRoadMapInteractor> presenter;
    ...
    @Override
    protected FragmentRoadMapBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentRoadMapBinding.inflate(inflater, container, attachToParent);
    }
    ...
    protected String getToolbarTitle() {
        return getString(R.string.road_utility_location);
    }
    ...
    protected void init(View v) {
        ...
        getViewBinding().btnRoadMapNext.setOnClickListener(view -> {
            if (roadLength != null && roadLength.length() > 0 && geom!=null && geom.getCoordinates().size()>0) {
                getBaseActivity().launchRoadDetails(true, false, roadLength, geom);
            } else {
                showDialog(getString(R.string.road_utility_location), getString(R.string.err_road_utility_location), null, null);
            }
        });
    ...
    public void setRoadGeom(GeomPolyLine geom){
    ...
    private void setBaseMap() {
        ...
        if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
            ...
            layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getRoad());
        }
    }
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null &&
            ...
            roadLength = AppCacheData.getOurInstance().getBuildingAssetData().getRoadUtility().getLength();
            getViewBinding().includeSketchBar.lengthAreaRadiusText.setText(String.format(Locale.ENGLISH, "%s %s m",
                    getResources().getString(R.string.dashboard_length), roadLength));
            geom = AppCacheData.getOurInstance().getBuildingAssetData().getRoadUtility().getGeom();
            setRoadGeom(geom);
        }
            ...
            if (getViewBinding().includeSketchBar.sketchOk.isEnabled()) {
                ...
                if (sketchGeometry != null) {
                    if (sketchGeometry.getGeometryType() == GeometryType.POLYLINE) {
                        ...
                        roadLength = String.format(Locale.ENGLISH, "%.2f", polylineLength);
