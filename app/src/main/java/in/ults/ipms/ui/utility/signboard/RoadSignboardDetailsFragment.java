...
import in.ults.ipms.databinding.FragmentRoadSignboardDetailsBinding;
...
public class RoadSignboardDetailsFragment extends BaseFragment<FragmentRoadSignboardDetailsBinding> implements IRoadSignboardDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = RoadSignboardDetailsFragment.class.getSimpleName();

    public static final int ERROR_TYPE_PLACE = 1;
    public static final int ERROR_TYPE_CATEGORY = 2;
    ...
    public static RoadSignboardDetailsFragment newInstance() {
        return new RoadSignboardDetailsFragment();
    }

    @Inject
    IRoadSignboardDetailsPresenter<IRoadSignboardDetailsView, IRoadSignboardDetailsInteractor> presenter;


    CommonSpinnerAdapter<UtilitySpinnerResponse.Ward> wardAdapter;
    CommonSpinnerAdapter<UtilitySpinnerResponse.SignboardCategory> signboardCategoryAdapter;
    ...
    @Override
    protected FragmentRoadSignboardDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentRoadSignboardDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.road_signboard_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgRoadSignboard.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if(AppCacheData.getOurInstance().getUtilitySpinnerData()!=null && AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues()!=null) {
            ...
            signboardCategoryAdapter = CommonSpinnerAdapter.setAdapter(getBaseActivity(), getViewBinding().srRoadSignboardCategory, AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues().getSignboardCategory());
        }
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getRoadSignBoard());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvRoadSignboardDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvRoadSignboardDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        String place = Objects.requireNonNull(getViewBinding().etRoadSignboardPlace.getText()).toString().trim();
        String remarks = Objects.requireNonNull(getViewBinding().etRemarks.getText()).toString().trim();
        String category = (String) getViewBinding().srRoadSignboardCategory.getTag();
        String wardNo = (String) getViewBinding().srWardNo.getTag();
        presenter.validateData(place, category, wardNo, remarks, photo, latitude, longitude);
    }
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_PLACE:
                getViewBinding().layoutRoadSignboardPlace.setError(error);
                getViewBinding().layoutRoadSignboardPlace.requestFocus();
                break;
            case ERROR_TYPE_CATEGORY:
                getViewBinding().layoutRoadSignboardCategory.setError(error);
                getViewBinding().layoutRoadSignboardCategory.requestFocus();
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutRoadSignboardPlace.setErrorEnabled(false);
        getViewBinding().layoutRoadSignboardCategory.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null ) {
            UtilityAssets.RoadSignboard playgroundDetails = AppCacheData.getOurInstance().getBuildingAssetData().getRoadSignboardDetails();
            if (playgroundDetails != null) {
                getViewBinding().etRoadSignboardPlace.setText(playgroundDetails.getPlace());
                getViewBinding().etRemarks.setText(playgroundDetails.getRemarks());
                signboardCategoryAdapter.setContent(String.valueOf(playgroundDetails.getCategory()));
                wardAdapter.setContent(String.valueOf(playgroundDetails.getWard()));
                GeomPoint geom = playgroundDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+playgroundDetails.getPhoto1(), null, playgroundDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_ROAD_SIGNBOARD, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageUploadSuccess(String imagePath, String imageType, String response) {
        ...
        Glide.with(this)
                ...
                .into(getViewBinding().imgRoadSignboard);
    }
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgRoadSignboard.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
