...
import in.ults.ipms.databinding.FragmentStatueDetailsBinding;
...
public class StatueDetailsFragment extends BaseFragment<FragmentStatueDetailsBinding> implements IStatueDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = StatueDetailsFragment.class.getSimpleName();
    ...
    public static final int ERROR_TYPE_STATUE_LOCATION = 5;

    public static StatueDetailsFragment newInstance() {
        return new StatueDetailsFragment();
    }

    @Inject
    IStatueDetailsPresenter<IStatueDetailsView, IStatueDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentStatueDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentStatueDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.statue_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgStatue.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getStatue());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvStatueDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvStatueDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        ...
        presenter.validateData(location, wardNo, remarks, photo, latitude, longitude);
    }
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_LOCATION:
                getViewBinding().layoutLocation.setError(error);
                getViewBinding().layoutLocation.requestFocus();
                break;
            ...
            case ERROR_TYPE_STATUE_LOCATION:
                showToast(error);
                break;
            default:
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutLocation.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null ) {
            UtilityAssets.Statue statueDetails = AppCacheData.getOurInstance().getBuildingAssetData().getStatueDetails();
            if (statueDetails != null) {
                getViewBinding().etLocation.setText(statueDetails.getLocation());
                getViewBinding().etRemarks.setText(statueDetails.getRemarks());
                wardAdapter.setContent(String.valueOf(statueDetails.getWard()));
                GeomPoint geom = statueDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+statueDetails.getPhoto1(), null, statueDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_STATUE, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgStatue.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
