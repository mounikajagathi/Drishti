...
import in.ults.ipms.databinding.FragmentStreetTapDetailsBinding;
...
public class StreetTapDetailsFragment extends BaseFragment<FragmentStreetTapDetailsBinding> implements IStreetTapDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = StreetTapDetailsFragment.class.getSimpleName();

    public static final int ERROR_TYPE_LOCATION = 1;
    public static final int ERROR_TYPE_ADDRESS = 2;
    ...
    public static final int ERROR_TYPE_STREET_LOCATION = 6;
    public static final int ERROR_TYPE_FUNDED_BY = 7;
    public static final int ERROR_TYPE_WORKING_STATUS = 8;

    public static StreetTapDetailsFragment newInstance() {
        return new StreetTapDetailsFragment();
    }

    @Inject
    IStreetTapDetailsPresenter<IStreetTapDetailsView, IStreetTapDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentStreetTapDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentStreetTapDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.street_tap_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgStreetTap.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getStreetTap());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvStreetTapDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvStreetTapDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        String place = Objects.requireNonNull(getViewBinding().etStreetTapLocation.getText()).toString().trim();
        String remarks = Objects.requireNonNull(getViewBinding().etRemarks.getText()).toString().trim();
        String address = Objects.requireNonNull(getViewBinding().etStreetTapAddress.getText()).toString().trim();
        String fundedBy = Objects.requireNonNull(getViewBinding().etStreetTapFundedBy.getText()).toString().trim();
        boolean workingStatus = getViewBinding().cbWorkingStatus.isChecked();
        String wardNo = (String) getViewBinding().srWardNo.getTag();
        presenter.validateData(place, workingStatus, fundedBy, address, wardNo, remarks, photo, latitude, longitude);
    }
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_LOCATION:
                getViewBinding().layoutStreetTapLocation.setError(error);
                getViewBinding().layoutStreetTapLocation.requestFocus();
                break;
            case ERROR_TYPE_ADDRESS:
                getViewBinding().layoutStreetTapAddress.setError(error);
                getViewBinding().layoutStreetTapAddress.requestFocus();
                break;
            ...
            case ERROR_TYPE_FUNDED_BY:
                getViewBinding().layoutStreetTapFundedBy.setError(error);
                getViewBinding().layoutStreetTapFundedBy.requestFocus();
                break;
            case ERROR_TYPE_WORKING_STATUS:
                getViewBinding().cbWorkingStatus.setError(error);
                getViewBinding().cbWorkingStatus.requestFocus();
                break;
            case ERROR_TYPE_PHOTO:
            case ERROR_TYPE_STREET_LOCATION:
                showToast(error);
                break;
            default:
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutStreetTapLocation.setErrorEnabled(false);
        getViewBinding().layoutStreetTapAddress.setErrorEnabled(false);
        getViewBinding().layoutWardNo.setErrorEnabled(false);
        getViewBinding().layoutRemarks.setErrorEnabled(false);
        getViewBinding().layoutStreetTapFundedBy.setErrorEnabled(false);
    }

    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null) {
            UtilityAssets.StreetTap streetDetails = AppCacheData.getOurInstance().getBuildingAssetData().getStreetTapDetails();
            if (streetDetails != null) {
                getViewBinding().etStreetTapLocation.setText(streetDetails.getLocation());
                getViewBinding().etRemarks.setText(streetDetails.getRemarks());
                getViewBinding().etStreetTapFundedBy.setText(streetDetails.getFundedBy());
                getViewBinding().etStreetTapAddress.setText(streetDetails.getAddress());
                getViewBinding().cbWorkingStatus.setChecked(streetDetails.isWorkingStatus());
                wardAdapter.setContent(String.valueOf(streetDetails.getWard()));
                GeomPoint geom = streetDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() + "" + streetDetails.getPhoto1(), null, streetDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_STREET_TAP, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageUploadSuccess(String imagePath, String imageType, String response) {
        ...
        Glide.with(this)
                ...
                .into(getViewBinding().imgStreetTap);
    }
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgStreetTap.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
