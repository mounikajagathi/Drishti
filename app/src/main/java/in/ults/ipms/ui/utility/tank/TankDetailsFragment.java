...
import in.ults.ipms.databinding.FragmentTankDetailsBinding;
...
public class TankDetailsFragment extends BaseFragment<FragmentTankDetailsBinding> implements ITankDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = TankDetailsFragment.class.getSimpleName();

    public static final int ERROR_TYPE_LOCATION = 1;
    public static final int ERROR_TYPE_TANK_OWNER = 2;
    public static final int ERROR_TYPE_TANK_TYPE = 3;
    public static final int ERROR_TYPE_CAPACITY = 4;
    public static final int ERROR_TYPE_REMARKS = 5;
    public static final int ERROR_TYPE_PHOTO = 6;
    public static final int ERROR_TYPE_TANK_LOCATION = 7;
    public static final int ERROR_TYPE_WARD = 8;
    ...
    @Inject
    ITankDetailsPresenter<ITankDetailsView, ITankDetailsInteractor> presenter;


    CommonSpinnerAdapter<UtilitySpinnerResponse.TankType> TankTypeAdapter;
    ...
    @Override
    protected FragmentTankDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentTankDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.tank_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgTank.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if(AppCacheData.getOurInstance().getUtilitySpinnerData()!=null && AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues()!=null) {
            ...
            TankTypeAdapter = CommonSpinnerAdapter.setAdapter(getBaseActivity(), getViewBinding().srTankType, AppCacheData.getOurInstance().getUtilitySpinnerData().getDropDownValues().getTankType());
        }
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                for (DashboardResponse.BaseLayers layer : baseLayerData) {
                    ...
                    String[] tankOwner = layer.getLayerName().split(":");
                    String layerName = tankOwner.length > 0 ? tankOwner[tankOwner.length - 1] : "";
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getTank());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvTankDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvTankDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        String location = Objects.requireNonNull(getViewBinding().etTankLocation.getText()).toString().trim();
        String tankOwner = Objects.requireNonNull(getViewBinding().etTankOwner.getText()).toString().trim();
        String remarks = Objects.requireNonNull(getViewBinding().etRemarks.getText()).toString().trim();
        String tankCapacity = Objects.requireNonNull(getViewBinding().etTankCapacity.getText()).toString().trim();
        String type = (String) getViewBinding().srTankType.getTag();
        String wardNo = (String) getViewBinding().srWardNo.getTag();
        presenter.validateData(location, tankOwner, tankCapacity, type, wardNo, remarks, photo, latitude, longitude);
    }
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_LOCATION:
                getViewBinding().layoutTankLocation.setError(error);
                getViewBinding().layoutTankLocation.requestFocus();
                break;
            case ERROR_TYPE_TANK_OWNER:
                getViewBinding().layoutTankOwner.setError(error);
                getViewBinding().layoutTankOwner.requestFocus();
                break;
            case ERROR_TYPE_TANK_TYPE:
                getViewBinding().layoutTankType.setError(error);
                getViewBinding().layoutTankType.requestFocus();
                break;
            case ERROR_TYPE_CAPACITY:
                getViewBinding().layoutTankCapacity.setError(error);
                getViewBinding().layoutTankCapacity.requestFocus();
                break;
            case ERROR_TYPE_WARD:
    ...
    public void clearErrors() {
        getViewBinding().layoutTankLocation.setErrorEnabled(false);
        getViewBinding().layoutTankOwner.setErrorEnabled(false);
        getViewBinding().layoutTankType.setErrorEnabled(false);
        getViewBinding().layoutTankCapacity.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null ) {
            UtilityAssets.Tank tankDetails = AppCacheData.getOurInstance().getBuildingAssetData().getTankDetails();
            if (tankDetails != null) {
                ...
                GeomPoint geom = tankDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() +""+tankDetails.getPhoto1(), null, tankDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_TANK, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgTank.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
