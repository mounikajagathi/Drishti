...
import in.ults.ipms.databinding.FragmentTaxiStandDetailsBinding;
...
public class TaxiStandDetailsFragment extends BaseFragment<FragmentTaxiStandDetailsBinding> implements ITaxiStandDetailsView, BaseActivity.ImagePickListener {

    public static final String TAG = TaxiStandDetailsFragment.class.getSimpleName();

    public static final int ERROR_TYPE_LOCATION = 1;
    public static final int ERROR_TYPE_AUTHORISED = 2;
    ...
    public static final int ERROR_TYPE_TAXI_LOCATION = 7;
    public static final int ERROR_TYPE_AUTHORISATION_DETAILS = 8;
    public static final int ERROR_TYPE_CAPACITY = 9;

    public static TaxiStandDetailsFragment newInstance() {
        return new TaxiStandDetailsFragment();
    }

    @Inject
    ITaxiStandDetailsPresenter<ITaxiStandDetailsView, ITaxiStandDetailsInteractor> presenter;
    ...
    @Override
    protected FragmentTaxiStandDetailsBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, boolean attachToParent) {
        return FragmentTaxiStandDetailsBinding.inflate(inflater, container, attachToParent);
    }

    @Override
    protected String getToolbarTitle() {
        return getString(R.string.taxi_stand_details_title);
    }
    ...
    protected void init(View view) {
        ...
        getViewBinding().imgTaxiStand.setOnClickListener(v -> captureImage(REQUEST_CODE_IMAGE));
        ...
        if (AppCacheData.getOurInstance().getDashboardDetails() != null) {
            if (AppCacheData.getOurInstance().getDashboardDetails().getLocalBody() != null) {
                ...
                for (DashboardResponse.BaseLayers layer : baseLayerData) {
                    ...
                    String[] capacity = layer.getLayerName().split(":");
                    String layerName = capacity.length > 0 ? capacity[capacity.length - 1] : "";
                ...
                if (AppCacheData.getOurInstance().getUtilitySpinnerData() != null &&
                    ...
                    layers.add(AppCacheData.getOurInstance().getUtilitySpinnerData().getLayerDetails().getTaxiStand());
                }
            }
        }
        ...
        public boolean onTouch(View view, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    getViewBinding().nsvTaxiStandDetails.requestDisallowInterceptTouchEvent(true);
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    getViewBinding().nsvTaxiStandDetails.requestDisallowInterceptTouchEvent(false);
                    break;
            }
    ...
    void submitOnClick() {
        ...
        String authDetails = Objects.requireNonNull(getViewBinding().etAuthDetails.getText()).toString().trim();
        boolean authorised = getViewBinding().cbAuthorised.isChecked();
        String type = (String) getViewBinding().srParkingType.getTag();
        String wardNo = (String) getViewBinding().srWardNo.getTag();
        presenter.validateData(location, authorised, capacity, authDetails, type, wardNo, remarks, photo, latitude, longitude);
    }
    ...
    public void showErrors(int errorType, String error) {
        switch (errorType) {
            case ERROR_TYPE_LOCATION:
                getViewBinding().layoutTaxiStandLocation.setError(error);
                getViewBinding().layoutTaxiStandLocation.requestFocus();
                break;
            case ERROR_TYPE_AUTHORISED:
                getViewBinding().cbAuthorised.setError(error);
                getViewBinding().cbAuthorised.requestFocus();
                break;
            case ERROR_TYPE_PARKING_TYPE:
                getViewBinding().layoutTaxiStandParkingType.setError(error);
                getViewBinding().layoutTaxiStandParkingType.requestFocus();
                break;
            case ERROR_TYPE_AUTHORISATION_DETAILS:
                getViewBinding().layoutTaxiStandAuthDetails.setError(error);
                getViewBinding().layoutTaxiStandAuthDetails.requestFocus();
                break;
            case ERROR_TYPE_CAPACITY:
                getViewBinding().layoutTaxiStandCapacity.setError(error);
                getViewBinding().layoutTaxiStandCapacity.requestFocus();
                break;
    ...
    public void clearErrors() {
        getViewBinding().layoutTaxiStandLocation.setErrorEnabled(false);
        getViewBinding().layoutTaxiStandParkingType.setErrorEnabled(false);
        getViewBinding().layoutWardNo.setErrorEnabled(false);
        getViewBinding().layoutTaxiStandCapacity.setErrorEnabled(false);
        getViewBinding().layoutTaxiStandAuthDetails.setErrorEnabled(false);
    ...
    public void setEditData() {
        if (AppCacheData.getOurInstance().getBuildingAssetData() != null) {
            ...
            if (taxiStandDetails != null) {
                ...
                parkingTypeAdapter.setContent(String.valueOf(taxiStandDetails.getParkingType()));
                wardAdapter.setContent(String.valueOf(taxiStandDetails.getWard()));
                GeomPoint geom = taxiStandDetails.getGeom();
                ...
                onImageUploadSuccess(AppCacheData.getOurInstance().getImageBaseURL() + "" + taxiStandDetails.getPhoto1(), null, taxiStandDetails.getPhoto1());
            }
        }
    }
    ...
    public void onImageCaptured(String path, int reqTypeCode) {
        if (AppCacheData.getOurInstance().getDashboardDetails() != null &&
            ...
            presenter.uploadImage(path, AppConstants.IMAGE_PATH_TAXI_STAND, AppConstants.IMAGE_PATH_PHOTO1,
    ...
    public void onImageUploadSuccess(String imagePath, String imageType, String response) {
        ...
        Glide.with(this)
                ...
                .into(getViewBinding().imgTaxiStand);
    }
    ...
    public void onImageRemoved(int reqTypeCode) {
        photo = null;
        getViewBinding().imgTaxiStand.setImageResource(R.mipmap.ic_property_image_place_holder);
    }
